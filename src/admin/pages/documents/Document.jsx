...
import { deleteDocument, getAllDocument, getDocumentByUserId } from '../../../features/actions/uploadDocumentAction';
...
const Document = () => {
  ...
  const  {userData} = useSelector((state) => state.auth);console.log("userss",userData.user._id)
  const dispatch = useDispatch();

useEffect(()=>{
  dispatch(getDocumentByUserId(userData.user._id)).then((res)=>{
console.log("res",res)
  })
},[])
      ...
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="bg-white divide-y divide-gray-200">
            {documentData?.map((item)=>(
              <tr> <td className="px-6 py-4 text-sm font-medium max-w-xs">
        ...
        {item?.documentTitle}
            ...
            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
  <a href={item.document} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
    View Document
  </a>
</td>
            <td className="px-6">
