...
import { getBlog, updateBlog } from "../../../../features/actions/blogAction";
...
const EditBlog = () => {
  ...
  const { blog, isSuccess, isLoading, error } = useSelector((state) => state.blog);
  ...
  useEffect(() => {
    if (blog?.data) {
      ...
      if (blog.data.blogImage) {
        setImagePreview(blog.data.blogImage);
      }
    }
  }, [blog, reset]);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(updateBlog({ data:formData, id })).then(
      (res)=>{
